document.addEventListener('DOMContentLoaded', () => {
  ...
  // Carrega os currículos do localStorage
  const curriculosSalvos = JSON.parse(localStorage.getItem('curriculos')) || [];

  // Verifica se existem currículos
  if (curriculosSalvos.length === 0) {
    listaCurriculosContainer.innerHTML = '<p class="text-center col-12">Nenhum currículo salvo ainda.</p>';
    return;
  }

  // Cria os cards para cada currículo
  curriculosSalvos.forEach((curriculo) => {
    ...
    cardContent.addEventListener('click', () => {
      // Agora a função já foi definida quando é chamada aqui
